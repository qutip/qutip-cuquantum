...
from qutip_cuquantum.stochastic.batching import batch_copy
...
class StateType(Enum):
    ...
    BATCH_KET = 4  # Batch of column vector (N, 1)
    BATCH_BRA = 5  # Batch of row vector (1, N)
    BATCH_DM = 6   # Batch of density matrix (N, N)
...
def random_custate(shape):
    *hilbert, state_type = shape
    ...
    if state_type in [StateType.KET, StateType.BATCH_KET]:
        out = random_pure_custate(hilbert)
    elif state_type in [StateType.DM, StateType.BATCH_DM]:
        out =  random_mixed_custate(hilbert)
    elif state_type in [StateType.BRA, StateType.BATCH_BRA]:
        ket_state = random_pure_custate(hilbert)
        out = CuState(ket_state.base, shape=(ket_state.shape[1], ket_state.shape[0]), copy=False)
    else:
        raise ValueError(f"Unsupported state type: {state_type}")
    if state_type in [StateType.BATCH_KET, StateType.BATCH_DM, StateType.BATCH_BRA]:
        out = batch_copy(out, 3)
    return out
