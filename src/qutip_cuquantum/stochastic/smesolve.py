...
from qutip.solver.multitrajresult import MultiTrajResult, _TrajectorySum
# from qutip.solver.sode.ssystem import StochasticOpenSystem, StochasticClosedSystem
from qutip.solver.sode._noise import PreSetWiener
from qutip.solver.result import Result, ExpectOp
from qutip.solver.multitraj import _MultiTrajRHS, MultiTrajSolver
...
class StochasticTrajResult(Result):
    ...
    def add(self, t, state, noise=None):
        ...
        if isinstance(state.data, CuState):
            self.batch_size = state.data.base.batch_size
